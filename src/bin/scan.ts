...
export async function main(ns: NS): Promise<void> {
    ...
    const workers = discoverHackedHosts(ns).map((host) => ns.ps(host).filter(p => p.filename === '/bin/autohack/worker.js').length).reduce((a, b) => a + b, 0);
    ...
    for (const host of hosts) {
        ...
        ns.tprint(`${host}; fullGrowTime=${fmt.time(ns, fullGrowTime)}; maxMoney=${fmt.money(ns, maxMoney)}; hackTime=${fmt.time(ns, hackTime)}; maxHackMoneyPerSec=${fmt.money(ns, maxHackMoneyPerSec)}; growTime=${fmt.time(ns, growTime)}; availableMoney=${fmt.money(ns, ns.getServerMoneyAvailable(host))}`);
    }
}