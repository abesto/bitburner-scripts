...
export async function main(ns: NS): Promise<void> {
  ...
  async function buyFor(money: number, print: (msg: string) => void): Promise<void> {
    let spent = 0;
    ...
    while (spent < money && n++ < 100) {
      ...
      if (score(target) < 0.6) {
        print(`Best stock to buy: ${target} has score ${fmt.float(score(target))}. That's < 0.6, not buying`);
        break;
      }

      const buyin = stonks * ns.stock.getBidPrice(target);
      print(
        `Best stonk to buy: ${target} has score ${fmt.float(score(target))}. Buying ${stonks} for ${fmt.money(buyin)}`,
      );
